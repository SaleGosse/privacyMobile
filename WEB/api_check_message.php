...
if(isset($_POST['userID']))
{
	...
	$rq_get_notif = "SELECT ms.idMessageStatus AS id,c.convName,p.firstName,m.content,m.date,m.idConversation AS conversationID FROM MessageStatus ms JOIN Message m ON m.idMessage = ms.idMessage JOIN Conversation c ON m.idConversation = c.idConversation JOIN Profile p ON m.idUser = p.idUser WHERE ms.unread = 1 AND ms.idUser = :userID";
	$rq_update_notif = "UPDATE MessageStatus SET unread = 0 WHERE idMessageStatus = :id";
